...
import com.example.demo.dto.StaffWorkCreateRequest;
...
public class StaffDashboardController {
    ...
    @PostMapping(value = "/me/works", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> createWork(
            @RequestHeader(value = "Authorization", required = false) String authorization,
            @ModelAttribute StaffWorkCreateRequest request
    ) {
        try {
            return ResponseEntity.ok(staffService.createWork(authorization, request));
        } catch (Exception e) {
            return authOrBadRequest(e);
        }
    }

    @GetMapping("/me/work-bookings")
    public ResponseEntity<?> getWorkBookings(@RequestHeader(value = "Authorization", required = false) String authorization) {
        try {
            return ResponseEntity.ok(staffService.getWorkBookings(authorization));
        } catch (Exception e) {
            return authOrBadRequest(e);
        }
    }
